...
const category = (p: string | null, ref: string) =>
  membership({ type: p ? 'MEMBER' : 'TRIAL', firstPurchasedAt: p ? date(p) : null }, date(ref))
    .membershipCategory;
test('membership follows Melbourne calendar days, including same-day lessons before purchase', () => {
  ...
  assert.equal(category(p, '2026-09-16T23:59:59'), 'MEMBER');
...
test('membership boundaries use 167/169 hour DST weeks', () => {
  for (const [start, end, hours] of [
    ...
    assert.equal(
      membership({ type: 'MEMBER', firstPurchasedAt: date(start) }, prior).membershipCategory,
      'NEW_MEMBER',
    );
