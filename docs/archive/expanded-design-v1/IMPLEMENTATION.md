...
基础建档仅创建 Student（姓名、年级及当前顾问负责），允许暂缺联系人；完整咨询步骤先补 Contact 关联，再由事务创建 Inquiry 和咨询 FollowUp。负责人只能取当前顾问；本阶段用 ACTIVE/ARCHIVED 管理档案可用性，不把它与销售意向混为一个状态。Inquiry.courseId 与 ClassSession.courseId 必须一致，Inquiry.studentId 必须属于目标学生。至少有一个可联系的主要联系人，姓名不能充当学生唯一标识；同姓名学生可存在。
...
并发写操作采用 Serializable 和有界重试；全局锁顺序为受影响 Teacher User（排课写时）、Student、ClassSession、权益／账户，每组按 ID 升序。单次预约无需教师锁，先锁 Student、再锁 ClassSession，锁内复查权限、试听资格、时间冲突和席位后写入；具体约定见 plan/00-contracts.md。课次状态为 SCHEDULED 或 CANCELLED；日期已过去不代表结果已提交。SCHEDULED 试听即使已过期仍是待处理记录，必须完成结果补录后才能再约同课程。
...
顾问基础概览仅返回姓名、年级、负责人和档案状态；联系人、咨询、试听详细反馈、跟进记录及 AI 草稿均需负责人权限。老师只能看到自己课次的姓名、试听标签、必要课程意向和教学反馈。不能把完整数据库对象序列化后再交给前端隐藏。

联系人可能被不同顾问负责的兄弟姐妹共用：新增家长管理只搜索当前顾问负责学生关联的联系人，可关联本人可见的已有家长，返回孩子列表也按权限裁剪。共享联系人存在其他负责人关联学生时禁止修改共享字段，返回 CONTACT_SHARED_LOCKED；不提供跨顾问全局搜索、合并或共享字段修改。不能因为能修改一个学生就间接修改另一个学生的联系方式。
...
目标导航、逐页权限、API DTO、数据库迁移、事务及验收拆分见 [plan/README.md](./plan/README.md)。任务00是共同开发合同。Admin拥有全部管理页面入口但业务敏感内容仍需负责人权限；Teacher仅自己的课次和学生。菜单管理不能创建任意路由或改变服务端授权。账号管理须处理停用撤销会话、最后一位管理员保护和已有业务关联。

当前新增基础学生GET/POST，不代表联系人、完整咨询或资源管理全部完成；系统账号/菜单/操作记录仍在计划中。取消后的SessionParticipant后续增加ACTIVE/REMOVED以留历史，所有人数与权限关联查询同时更新。正式入班、订单和扣课放在P2，避免分散试听闭环实现。
