...
**05d已完成（2026-09-18）：**Teacher课程详情新增点名弹窗，试听生置顶、逐行签到/补签，只有服务端确认后显示对勾及时间，顶部显示已签到人数。失败行保留错误可重试，其他行不受影响；30秒刷新名单并在开课/下课边界刷新资格。课前只读，隐藏全局老师筛选；课程详情移除旧整课反馈入口。Chrome验证与消费/审计核对、类型检查、9项单元测试及构建通过，见[点名页面验收](docs/ROLLCALL-UI-MILESTONE.md)。下一项06a逐学生评价；新教师待办列表/计数仍待07c，不把签到页面当作完整评价跟进闭环。

**06b已完成：**新增逐学生签到接口，仅当前授课Teacher可在开课后操作；签到、对应卡唯一扣1、参与及课次版本和审计同事务，重复请求不双扣。下课未签到预约不再占用，课后补签重新检查可用量；余额详情、列表和写入校验统一使用同次Clock快照。签到前购课取消隐藏试听评价，签到后购课保留教学义务。名单新增canCheckIn及签到时间/操作者，教师不获得余额明细。隔离PG、类型检查、单元测试、构建和OpenAPI生成通过，见[签到命令验收](docs/CHECKIN-COMMAND-MILESTONE.md)。点名弹窗已由05d完成，个人评价命令和待办列表计数仍由06a/07c实现。

**05c已完成：**名单仅保留加入与移除；取消记录只读，重新预约通过普通Add，跨课程调整分两步执行。删除Move/Restore、目标课程选择与重约来源锁定/透传，旧来源URL参数自动清理。联系人不完整不拦预约，双池余额及不足补课返回继续保留；本人学生、重复加入、提交中和课时校验仍有效。Chrome与数据库核对通过，类型检查、9项单元测试及构建通过，见[名单页面验收](docs/BOOKING-UI-SIMPLIFICATION-MILESTONE.md)。旧公开接口由09d清退，签到命令已由06b完成。

**04f已完成：**名单加入/移除不再生成重约；已取消且未消费的参与行可通过Add重新加入并重新选卡。只保留学生时间冲突、权限、状态、同课唯一和对应卡可用量；加入试听身份学生预建一条隐藏评价，取消关闭、重加复用，会员用试听卡不产生试听任务。签到命令已由06b完成，待办列表筛选由07c接入，名单界面已由05c清理，见[预约简化验收](docs/BOOKING-SIMPLIFICATION-MILESTONE.md)。
...
**01g已完成：**新增逐学生教师评价任务类型、签到/个人评价时间字段、独立跟进结果及沟通顾虑/核心问题/原因标签；后者可通过现有沟通API读写，类型已生成。两份新增迁移已在隔离PG及带旧数据升级场景通过，见[验收记录](docs/CHECKIN-CONTRACT-MILESTONE.md)。签到命令和点名弹窗已由06b/05d完成；隐藏待办列表筛选、个人评价以及“所有人工结果均完成”的跟进命令尚未实现；旧功能仍保留到后续任务收敛。运行新代码前须对目标开发库执行现有`pnpm db:migrate`，本轮未迁移18080环境或已有预览数据库。
