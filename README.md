...
**开发进度：已完成权益后端阶段，尚未整体切换。** 学生建档可选赠1节、双池摘要/流水、试听追加、正式自定义/套餐发放和首购关单已实现并在隔离PG验证，见[阶段验收](docs/ENTITLEMENT-MILESTONE.md)。建档表单已适配但待浏览器验收；三Tab与课时管理页面尚未完成。旧预约与反馈仍使用单科资格和人工报名日期，尚未接余额校验和扣课，新旧行为不能作为一个可发布版本混用。原用户数据库未迁移，旧seed也尚未适配权益版；请按01d/01e核对数据、09c协调切换，不直接将中间版本部署到现有数据环境。

## 已实现（原版）

- 简洁账号/密码登录，左菜单「我的待办 / 课表 / 学生」。Cookie 会话、CSRF、服务端角色和对象权限。
- 学生档案、主要家长联系方式、学习目标/偏好、独立沟通记录；原入学日仅作历史只读信息。Admin 只编辑本人学生；Teacher 不接收联系方式或销售记录。
