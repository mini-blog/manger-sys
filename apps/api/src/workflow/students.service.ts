...
export class StudentsService {
  ...
  fields(body: D.CreateStudentDto | D.UpdateStudentDto) {
    const { expectedVersion: _, ...fields } = body as D.UpdateStudentDto;
    ...
    return {
      ...fields,
      firstEnrolledOn: fields.firstEnrolledOn ? dateOnly(fields.firstEnrolledOn) : undefined,
    };
  }
  ...
  update(user: Actor, id: string, body: D.UpdateStudentDto, key?: string) {
    return this.commands.run(
      ...
      async (tx) => {
        ...
        if (
          s.firstEnrolledOn &&
          data.firstEnrolledOn &&
          s.firstEnrolledOn.getTime() !== data.firstEnrolledOn.getTime()
        )
          fail('ENROLMENT_DATE_LOCKED', 'The original enrolment date cannot be overwritten.');
        await this.validateEnrolment(tx, id, data.firstEnrolledOn);
        const merged = { ...s, ...data };
  ...
  async validateEnrolment(tx: Tx, id: string, date?: Date) {
    ...
    if (
      rows.some(
        (p) =>
          DateTime.fromJSDate(p.session.startsAt, { zone: 'Australia/Melbourne' }).toISODate()! <
